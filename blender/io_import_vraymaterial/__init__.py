...
class IMPORT_OT_vray_material(Operator, AddObjectHelper):
    ...
    def execute(self, context):
        ...
        for vrsf in vrscene_files:
            bpy.ops.vray.import_material(file_path=vrsf)
        return {'FINISHED'}


class importVRMaterial (bpy.types.Operator):
    """Change Dupli Group to other group"""
    bl_idname = "vray.import_material"
    bl_label = "Change Group"

    file_path = StringProperty(name="File Path", description="File Path", default="")
    ...
    def execute(self,context):
        ...
        for mn in mat.node_tree.nodes:
            mat.node_tree.nodes.remove( mn )
